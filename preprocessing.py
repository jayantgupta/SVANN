...
def generate_imagery_tiles(tile_width, tile_height, in_dir_root, in_folder):
...
def generate_mask_tiles(tile_width, tile_height, in_dir_root, in_folder):
	current_dir = os.path.join(in_dir_root, in_folder)
	for filename in tqdm(os.listdir(current_dir)):
		in_file = os.path.join(current_dir, filename)
		if os.path.isdir(in_file) and filename != 'tiles':
...
def partition_inputs(tile_width, tile_height, 
			imagery_dir_root=json.load(open('config.json'))['filepaths']['default_reprojections_dir'],
			mask_dir_root=json.load(open('config.json'))['filepaths']['default_masks_dir'],
			in_folder=''): # Assuming the imagery and masks have the same file structure to make filepaths manageable
	generate_imagery_tiles(tile_width, tile_height, imagery_dir_root, in_folder)
	generate_mask_tiles(tile_width, tile_height, mask_dir_root, in_folder)
...
def mask_generation(in_dir_root=json.load(open('config.json'))['filepaths']['input_imagery_dir'],
					in_folder='FL_raster_Miami_Dade_South',
	...
	for filename in tqdm(os.listdir(current_dir)):
		in_file = os.path.join(current_dir, filename)
		if os.path.isdir(in_file) and filename != 'tiles':
			# Recursively call mask_generation for processing folder trees
